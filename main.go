...
func register() {
	...
	if err == nil {
		...
		if conn, err := net.Dial(config.Type, config.Host+":"+config.Port); err == nil {
			...
			fmt.Print("REQUESTED: ", string(request))
			conn.Write(request)
...
func command() {
	...
	getHash := getCmd.String("hash", "", "The hash of the file to be retrieved")
...
func get(hash string) error {
	...
	if err == nil {
		...
		for scanner.Scan() {
			message := scanner.Bytes()
			json.Unmarshal(message, &desc)
			fmt.Print("RESPONSE: ", string(message)) 
		}
...
func add(file string, push bool) string {
	...
	//clone file to zeta default content repository
	copyToUniverse(file)
...
func saveHashGlobally(file string, hash string) error {
	...
	if err == nil {
		...
		desc, _ := describeInBytes(file, hash)
...
func saveHashLocally(file string) (string, error) {
	...
	desc, err := describeInBytes(file, hash)
...
func hashDescription(desc *common.Desc) string {
	b, _ := json.Marshal(desc)
	return utils.HashContent(b)
}

//DescribeInBytes returns file description and hash
func describeInBytes(file string, hash string) ([]byte, error) {
	f, err := os.Open(file)
	...
	desc := common.Desc{Fp: file, Ha: "md5", Le: fi.Size(), Ep: endpoint, Hs: hash}
